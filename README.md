The Switch Controller Companion!

An open source FPGA based USB adapter to connect a Nintendo Gamecube controller to Switch 2|1 and PC. The adapter in Switch mode mimics the official Nintendo Gamecube NSO controller, or an official Nintendo Gamecube adapter for Smash Bros., but with significantly less latency. This adapter is made with Gamecube NSO in mind, custom joystick calibrations, allowing for improved joystick control in games like F-Zero GX and camera controls in Zelda The Wind Waker. Total cost of project from the FPGA board used (Tang Nano 9k), Gamecube connectors, PCB order, and 3D printed parts is currently at $45 per adapter.
...
2. Acts as a wired Gamecube NSO controller, a Switch 2 Pro Controller, XBOX360 output for PC, and a Smash Bros. Gamecube Adapter with native Dolphin support.
